  ...
  openProductDetails(productId, shouldNavigate = true) {
    ...
    if (tableContainer) {
      const cols = (product.columns || [
        ...
        { key: "rangeDb", label: "Диапазон", suffix: " мм", check: (v) => v.rangeDb && v.rangeDb !== "" },
        { key: "wallDb", label: "Стена", suffix: " мм", check: (v) => v.wallDb && v.wallDb !== "" },
        { key: "pressure", label: "Работно нал.", suffix: " Bar", check: (v) => parseFloat(v.pressure) > 0 },
        { key: "burstDb", label: "Разрушаващо нал.", check: (v) => v.burstDb && v.burstDb !== "" },
        { key: "vacuumDb", label: "Вакуум", check: (v) => v.vacuumDb && v.vacuumDb !== "" },
        { key: "spacingDb", label: "Разст. зъби", suffix: " мм", check: (v) => v.spacingDb && v.spacingDb !== "" },
        { key: "hexDb", label: "HEX размер", suffix: " мм", check: (v) => v.hexDb && v.hexDb !== "" },
        { key: "braidsDb", label: "Оплетки", check: (v) => v.braidsDb && v.braidsDb !== "" },
        { key: "sleeveWidthDb", label: "Шир. ръкав", suffix: " мм", check: (v) => v.sleeveWidthDb && v.sleeveWidthDb !== "" },
